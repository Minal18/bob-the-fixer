# Bob the Fixer

An AI-powered GitHub PR reviewer that catches repetitive, low-judgment issues — missing null checks, inconsistent error handling, obvious security gaps, style deviations — before a human reviewer gets to the PR. Bob supplements human review; it doesn't approve or block merges.

Teams opt in per repo, configure what Bob checks for via a config file, and get inline PR comments powered by an LLM.

## Docs

- [PRD](./docs/PRD.md) — problem statement, goals, scope, success metrics
- [Tech Design](./docs/TECH_DESIGN.md) — architecture, components, data model

## Status

v1 scope — not yet enabled by default anywhere.
